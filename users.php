<?php
  ...
  include 'header.php';
  // $query = $conn->query("select * from users"); // uncomment untuk user master
  ...
  if ($op == 'delete') {
    $id = $_GET['id'];
    // $sql1 = $conn->query("delete from users where id= '$id'");
  ...
  if ($op == 'edit') {
    $id = $_GET['id'];
    // $sql2 = $conn->query("select * from users where id = '$id'");
  ...
  if (isset($_POST['simpan'])) { //untuk create
    ...
    if ($name) {
        if ($op == 'edit') { //untuk update
            // $query_edit = $conn->query("update users set name = '$name', username = '$username', password = '$password', phone = '$phone', location = '$location', role_id = '2' where id = '$id'");
          ...
          // $query_insert = $conn->query("insert into users (name, username, password, phone, location, role_id) values ('$name', '$username', '$password', '$phone', '$location', '2')");
  ...
  <div class="mx-auto">
    ...
    <div class="card">
      ...
      <div class="card-body">
        ...
        <?php
            header("refresh:3;url=users.php");//3 : detik
        }
        ?>
        <?php
        if ($sukses) {
        ...
        <table class="table">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td scope="row">
                <a href="users.php?op=edit&id=<?php echo $value['id'] ?>"><button type="button" class="btn btn-warning">Edit</button></a>
                <a href="users.php?op=delete&id=<?php echo $value['id']?>" onclick="return confirm('Yakin mau delete data?')"><button type="button" class="btn btn-danger">Delete</button></a>            
              </td>
            </tr>
            <?php
              }
