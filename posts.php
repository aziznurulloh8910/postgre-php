<?php
  ...
  include 'header.php';
  // $query = $conn->query("select * from posts"); //uncomment untuk posts
  ...
  if ($op == 'delete') {
    $id = $_GET['id'];
    // $sql1 = $conn->query("delete from posts where id= '$id'");
  ...
  if ($op == 'edit') {
    $id = $_GET['id'];
    // $sql2 = $conn->query("select * from posts where id = '$id'");
  ...
  if (isset($_POST['simpan'])) { //untuk create
    ...
    if ($description) {
        if ($op == 'edit') { //untuk update
            // $query_edit = $conn->query("update posts set description = '$description', location = '$location', user_id = '$user_id' where id = '$id'");
            ...
            // $query_insert = $conn->query("insert into posts (description, location, user_id) values ('$description', '$location', '$user_id')");
  ...
  <div class="mx-auto">
    ...
    <div class="card">
      ...
      <div class="card-body">
        ...
        <?php
            header("refresh:3;url=posts.php");//3 : detik
        }
        ?>
        <?php
        if ($sukses) {
        ...
        <table class="table">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td scope="row">
                <a href="posts.php?op=edit&id=<?php echo $value['id'] ?>"><button type="button" class="btn btn-warning">Edit</button></a>
                <a href="posts.php?op=delete&id=<?php echo $value['id']?>" onclick="return confirm('Yakin mau delete data?')"><button type="button" class="btn btn-danger">Delete</button></a>            
              </td>
            </tr>
            <?php
              }
